...
#setwd('../../')
#setwd('../../')
#setwd('03_plots/description/numerical') <- path to save files
...
create_scatterplot <- function(df, x_column, detail, plot_title) {
  ...
  # Plot points
  gg <- ggplot(filtered_df, aes_string(x = x_column, y = "Logistic.Fit", color = "technology")) +
  ...
  plot_filename <- paste0("Scatterplot_", plot_title, ".png")
  ggsave(file.path(plot_filename), plot = gg, device = "png", width = 12, height = 12, dpi = 300)
  print(gg)
  return(gg)
}
...
create_scatterplot_poly <- function(df, x_column, detail, plot_title) {
  ...
  if (x_column %in% c("Average.lifetime", "Material.Use.Numerical", "Granularity.Numerical")) {
    ...
    gg <- gg + 
      geom_abline(intercept = coef(lm_fit)[1], slope = coef(lm_fit)[2], color = "sienna", linetype = "solid", size = 1.2) +
      annotate("text", x = Inf, y = Inf, label = paste("R² (Linear) =", round(summary(lm_fit)$r.squared, 4), 
                                                       "\nn = ", sum(!is.na(x)), 
                                                       "\nLinear Regression of Log(", x_column, ") and Logistic Fit"), 
               hjust = 1, vjust = 135, size = 5, color = "sienna")
  } else {
    if (!is.na(x_column)) {
      x <- filtered_df[[x_column]]
      y <- filtered_df[["Logistic.Fit"]]
      ...
      gg <- gg + 
        geom_abline(intercept = coef(lm_fit)[1], slope = coef(lm_fit)[2], color = "sienna", linetype = "solid", size = 1.2) +
        annotate("text", x = Inf, y = Inf, label = paste("R² (Linear) =", round(summary(lm_fit)$r.squared, 4), "\nn = ", sum(!is.na(x))), 
                 hjust = 1, vjust = 1.45, size = 5, color = "sienna")
    }
  }
  
  # Add polynomial regression line (degree 2) (dashed)
  poly_fit <- lm(Logistic.Fit ~ poly(filtered_df[[x_column]], 2, raw = FALSE), data = filtered_df)
  gg <- gg + 
    geom_smooth(method = "lm", formula = y ~ poly(x, 2, raw = FALSE), se = FALSE, color = "mediumorchid", linetype = "dashed", size = 1.2) +
    annotate("text", x = Inf, y = Inf, label = paste("R² (Polynomial) =", round(summary(poly_fit)$r.squared, 4)), 
             hjust = 1, vjust = 1.1, size = 5, color = "mediumorchid")
...
create_scatterplot_cubic <- function(df, x_column, detail, plot_title) {
  ...
  if (x_column %in% c("Average.lifetime", "Material.Use.Numerical", "Granularity.Numerical")) {
    ...
    gg <- gg + 
      ...
      annotate("text", x = Inf, y = Inf, label = paste("R² (Linear) =", round(summary(lm_fit)$r.squared, 2), 
                                                       "\nn = ", sum(!is.na(x)), 
                                                       "\nLinear Regression of Log(", x_column, ") and Logistic Fit"), 
               hjust = 1, vjust =7.5, size = 5, color = "sienna")
  } else {
    if (!is.na(x_column)) {
      x <- filtered_df[[x_column]]
      y <- filtered_df[["Logistic.Fit"]]
  ...
  gg <- gg + 
    ...
    annotate("text", x = Inf, y = Inf, label = paste("R² (Polynomial) =", round(summary(poly_fit)$r.squared, 2)), 
             hjust = 1, vjust = 5.5, size = 5, color = "mediumorchid")
  
  #Add cubic regression line (degree 3) (solid dark blue) ---
  poly_cubic_fit <- lm(Logistic.Fit ~ poly(filtered_df[[x_column]], 3, raw = FALSE), data = filtered_df)
  gg <- gg + 
    geom_smooth(method = "lm", formula = y ~ poly(x, 3, raw = FALSE), se = FALSE, color = "darkblue", linetype = "solid", size = 1.2) +
    annotate("text", x = Inf, y = Inf, label = paste("R² (Cubic) =", round(summary(poly_cubic_fit)$r.squared, 2)), 
             hjust = 1, vjust = 1.5, size = 5, color = "darkblue")
  ...
  # Save and show the plot
  # Note: Moving these before the final return so they actually run
  plot_filename <- paste0("cubic_Scatterplot_", plot_title, ".png")
  ggsave(file.path(plot_filename), plot = gg, device = "png", width = 12, height = 12, dpi = 300)
  print(gg)
