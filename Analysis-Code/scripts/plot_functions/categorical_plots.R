...
describe_categorical_tech <- function(data, x_var, y_var, category_var, category_order, label_vars, plot_title, save_filename) {
  ...
  histograms <- lapply(category_order, function(category) {
    ...
    # Save each histogram
    clean_category <- str_replace_all(category, "[/ ]", "_") # Replaces slashes and spaces
    file_name <- paste0("Update_histogram_", category_var, "_", clean_category, ".png")
    ggsave(file_name,
           plot = hist_plot,
           width = 6, height = 6, units = "in", dpi = 300)
